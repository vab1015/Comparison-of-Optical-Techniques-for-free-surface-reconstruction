%% Computing free surface elevations using Optic flow and FS-SS 
%  See Moisy et al. (2009) for details of FS-SS setup and equations used herein. 
...
tic
vidReader = VideoReader('FSSS_image_stack_J3.avi');  %Video is an image stack with every odd numbered frame being the reference checkerboard. 
...
%Loop below is run in a way so that only even elements of flowp are considered.
...
%Paramters used to compute surface gradients - See Moisy et al. (2009)  
alpha= 0.24; %Parameter related to the refractive indices of 2 fluids. Specified value is for an air-water interface. 
h_p = 0.221; %Effective surface-pattern distance [m]
H= 1.56;  %Pattern-camera distance [m]
den = ((1/(alpha*h_p)) - (1/H));  %Term denoted by 1/h* in Eqn 3 of Moisy et al. (2009) 
...
for jj=1:N
    ...
    v_calib = final_V_cell_data{jj,1}.*(L_x/size(final_U_cell_data{1,1},1));  %Converting y displacements from pixel to real-world coordinates
    fx = u_calib .*-(den);     %Computing surface gradients in x direction (See Eqn 13 from Moisy et al. (2009) 
    fy = v_calib .*-(den);     %Computing surface gradients in y direction (See Eqn 13 from Moisy et al. (2009) 
...
myVideo = VideoWriter('FSSS Free-surface J1');
myVideo.FrameRate= 5;
open(myVideo)
...
FSSS_output.U_disp = final_U_cell_data; 
FSSS_output.V_disp = final_V_cell_data; 
FSSS_output.eta = eta; 

save FSSS_output.mat FSSS_output -v7.3 
